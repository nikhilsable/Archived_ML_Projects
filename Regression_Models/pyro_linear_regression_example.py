...
def bay_lin_reg_pyro(df, pred_for_days):
    ...
    class BayesianRegression(PyroModule):
        def __init__(self, in_features, out_features):
            ...
            self.linear.weight = PyroSample(dist.Normal(0.0, 1.0).expand([out_features, in_features]).to_event(2))
            self.linear.bias = PyroSample(dist.Normal(0.0, 10.0).expand([out_features]).to_event(1))
    ...
    future_df = pd.DataFrame(
        index=pd.date_range(
            start=orig_df.index.min(),
            end=(orig_df.index.max() + pd.Timedelta(days=pred_for_days)),
...
def get_plotly_fig_ts_data(
    ...
    limits_titles = {
        "UEL": upper_error_limit,
        "UWL": upper_warning_limit,
    ...
    shapes = [
        {
            "type": "line",
            "x0": final_df.index.min().tz_localize(None),
            "y0": upper_warning_limit,
            "x1": final_df.index.max().tz_localize(None),
            "y1": upper_warning_limit,
            "line": {"color": "orange"},
        },
        {
            "type": "line",
    ...
    fig = {
        ...
        "layout": {
            "title": {
                "text": limits_titles["chart_title"],
                "font": {"family": "Courier New, monospace", "size": 24, "color": "#7f7f7f"},
            },
            "xaxis": {
                "title": {
                    "text": limits_titles["x_axis_title"],
                    "font": {"family": "Courier New, monospace", "size": 24, "color": "#7f7f7f"},
                },
                "range": [final_df.index.min(), final_df.index.max()],
            },
            "yaxis": {
                "title": {
                    "text": limits_titles["y_axis_title"],
                    "font": {"family": "Courier New, monospace", "size": 24, "color": "#7f7f7f"},
                }
            },
            "shapes": shapes,
    ...
    for value in range(0, len(final_df.columns)):
        fig["data"].append(
            {
                "type": "scatter",
                "x": pd.to_datetime(trace_dfs[value].index.ravel()),
                "y": trace_dfs[value].values.ravel(),
                "name": trace_names[value],
...
upper_warning_limit, upper_error_limit = (
    final_df.max().max(),
    final_df.max().max(),
)
