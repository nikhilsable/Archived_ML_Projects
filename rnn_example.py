...
def df_reindexer(df, timesteps="H"):
    new_idx = pd.date_range(
        start=df.index.min().tz_localize(None), end=df.index.max().tz_localize(None), freq=timesteps, tz="UTC"
    )
...
model = keras.models.Sequential([keras.layers.Flatten(input_shape=[n_steps, 1]), keras.layers.Dense(1)])
